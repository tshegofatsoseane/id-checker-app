<template>
        ...
        <v-card-text>
          ...
          <v-card v-if="result" outlined class="mt-5">
            ...
            <v-card-text>
              ...
              <!-- displaying holidays list -->
              <v-list dense>
                <v-list-item v-for="holiday in result.holidays" :key="holiday.holiday_name">
                  <v-list-item-content>
                    <v-list-item-title><strong>{{ holiday.holiday_name }}</strong></v-list-item-title>
                    <v-list-item-subtitle><strong>Date:</strong> {{ holiday.holiday_date }}</v-list-item-subtitle>
                    <v-list-item-subtitle><strong>Type:</strong> {{ holiday.holiday_type }}</v-list-item-subtitle>
                    <v-list-item-subtitle class="holiday-description"><strong>Description:</strong> {{ holiday.description }}</v-list-item-subtitle>
                  </v-list-item-content>
                </v-list-item>
              </v-list>
              ...
              <v-row justify="center" class="mt-4">
                <v-btn :disabled="!holidays.previous" @click="fetchHolidays(holidays.previous)">
                  Previous
                </v-btn>
                <v-btn :disabled="!holidays.next" @click="fetchHolidays(holidays.next)">
                  Next
                </v-btn>
              </v-row>
            </v-card-text>
  ...
  export default {
    ...
    methods: {
      ...
      // Search for ID number and fetch associated data
      async searchID() {
        try {
          ...
          this.holidays = {
            results: response.data.results.holidays,
      ...
      async fetchHolidays(url) {
        ...
        try {
          ...
          this.error = err.response?.data?.error || "An error occurred while fetching holidays";
        }
      },
    },
  };
